> 공개 저장소용 운영/점검 예시 메모
>
> 실제 조직 운영 매뉴얼이 아니라, 이 프로젝트를 재현하거나 데모할 때 참고할 수 있는 개인용 체크리스트입니다.
> 
...
- 수동 백업이 필요하면 관리자 대시보드의 "지금 백업 및 삭제 실행" 버튼으로 즉시 실행 가능
...
1. Google Sheets 열기
2. 파일 → 다운로드 → CSV로 다운로드
3. 로컬 저장: visitor-export-YYYY-MM-DD.csv
