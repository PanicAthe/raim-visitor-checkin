## 1. 프로젝트 개요

**프로젝트명**: 방문자 체크인 시스템

**작성일**: 2026-01-28

**수정일**: 2026-02-11

> 본 문서는 개인 포트폴리오 공개용으로 다듬은 요구사항 메모입니다. 실제 운영 조직의 내부 기준서나 계약 문서가 아니라, 구현 범위와 기술적 판단을 정리한 참고 자료입니다.

---

## 2. 프로젝트 요구사항

### 2.1 목표

- 방문자 입력 과정을 웹에서 간단히 처리
- 수집된 데이터를 백업 가능한 구조로 관리
- 무료 또는 저비용 도구만으로 구성 가능한지 검증
- 소규모 현장 환경에서 운용 가능한 흐름 설계

### 2.2 주요 문제 해결

- 수기 입력 과정의 번거로움
- 데이터 누락 가능성
- 별도 서버 운영 비용 부담

---

## 3. 기능 요구사항

### 3.1 필수 기능

| ID | 기능 | 설명 |
| --- | --- | --- |
| F001 | 웹 기반 입력 인터페이스 | 방문자가 웹 브라우저에서 접근 가능한 체크인 화면 |
| F002 | 카메라 기반 인식 | 웹캠을 통한 얼굴 인식으로 성별/연령대 자동 감지 |
| F003 | 최종 확인 모달 | AI 스캔결과가 나왔거나 수동입력 데이터 최종 확인, 취소/확인 버튼만 제공 |
| F004 | 수동 입력 모드 (기본값) | 기본 모드, AI 모드와 전환 가능, 방문자 직접 입력 |
| F005 | 방문자 리스트 | 전송 전 데이터 확인/편집, 그룹화 및 수량 조절 |
| F006 | 한/영 다국어 지원 | 한국어/영문 언어 선택 기능 |
| F007 | 관리자 잠금 화면 | 최초 진입 시 공간 선택 및 비밀번호 입력으로 인증 |
| F008 | 관리자 대시보드 | localStorage 기반 일일 통계 및 설정 관리 (로고 3회 터치, 공간별 필터링) |
| F009 | 자동 백업 | 설정된 주기마다 Firestore → Google Sheets 백업 |
| F010 | 자동 정리 | 백업 완료 후 Firestore 데이터 자동 삭제 |
| F011 | 수동 백업 및 삭제 | 관리자 대시보드에서 즉시 백업 및 삭제 실행 |
| F012 | 오류 알림 | 백업 실패 시 관리자 이메일 알림 |
| F013 | 배포 | Vercel을 통한 웹 배포 (Git 연동) |

---

## 4. 비기능 요구사항

### 4.1 성능

- 페이지 로딩 시간: 2초 이내 (AI 모델 로딩 시간 포함)
- 데이터 입력 응답시간: 1초 이내
- 동시 사용자: 10명 이상
- 일일 처리량: 1,500명+ (데모 시나리오 기준)

### 4.2 안정성

- 시스템 가용성: 99% 이상을 목표로 함
- 데이터 손실 위험 최소화 (백업 후 삭제 원칙)
- 백업 성공률: 99.5% 이상 목표
- 타임아웃 대응: 작업 중단 후 오류 알림 및 재시도 안내

### 4.3 확장성

- 일일 방문자 3,000명까지 무료 티어로 처리 가능한 구조를 목표로 함
- 트리거 간격 조정으로 구조 변경 없이 트래픽 대응 (1/3/6/12시간 선택)

### 4.4 보안

- Firebase Firestore 인증: 서비스 계정 기반
- 데이터 전송: HTTPS 암호화
- 접근 제어: 관리자 암호 보호

### 4.5 사용성

- 비개발자도 설정/운영 가능
- 설정 시간: 30분 이내
- 유지보수: 월 1회 수준의 가벼운 점검을 목표로 함

### 4.6 호환성

- 브라우저: Chrome, Safari, Firefox 최신 버전
- 운영체제: Windows, macOS, Linux
- 모바일: 반응형 웹 디자인 (스마트폰 접근 가능)

---

## 5. 시스템 요구사항

### 5.1 기술 스택

| 계층 | 기술 |
| --- | --- |
| Frontend | React + Vite |
| Face Recognition | face-api.js |
| Backend | Firebase Firestore |
| Automation | Google Apps Script |
| Backup | Google Sheets |
| Hosting | Vercel |

### 5.2 외부 의존성

- Firebase 프로젝트
- Google 계정
- Google Apps Script OAuth2 라이브러리
- face-api.js 모델 파일

### 5.3 인프라 비용

- 총 운영비: 0원/월
- 무료 한도 사용률: 40% 미만 (3시간 트리거 기준, 1/3/6/12시간 선택 가능)

---

## 6. 데이터 요구사항

### 6.1 저장 데이터

```
방문자 기록:
- timestamp: 날짜/시간
- gender: 성별 (남성/여성)
- ageGroup: 연령대 (10대, 20대, ...)
- source: 입력 방식 (AI/수동)
- location: 공간 구분값 (예: hall-a, room-b)

```

### 6.2 데이터 보관 정책

- localStorage: 일일 통계용 (앱 시작 시 과거 데이터 삭제)
- Firestore: 설정 간격마다 백업 후 삭제 (1/3/6/12시간)
- Google Sheets: 영구 보관

### 6.3 데이터 용량

- 일일 데이터: 약 1,000개 문서 (1,000명 기준)
- 월간 데이터: 약 30,000개 문서
- 연간 Google Sheets: 약 365만 셀 (1,000만 셀 제한의 36%)

---

## 7. 배포 요구사항

- Frontend: Vercel (Git 연동, GitHub push 시 자동 배포)
- Backend: Firebase Firestore (클라우드)
- 배포 시간: 2분 이내

---

## 8. 유지보수 요구사항

> 상세 모니터링 및 유지보수는 [MAINTENANCE_GUIDE.md](./MAINTENANCE_GUIDE.md)를 참조하세요.

- 일일: 오류 알림 여부 확인
- 주간: Google Sheets 데이터 및 Firebase 사용량 확인
- 분기별: 서비스 계정 키 보안 점검 (선택사항)

---

## 9. 제약사항 및 위험요소

### 9.1 제약사항

- Firebase 무료 티어: 1일 50,000회 읽기, 20,000회 쓰기/삭제
- Apps Script: 1회 6분 제한, 1일 90분 제한
- Google Sheets: 1,000만 셀 저장량
- 페이지 크기 제한: 한 번에 300 문서씩 조회 (pageSize=300), batchWrite 삭제는 500개/배치

### 9.2 확장 시나리오

- 일일 3,000명+ 지속적 유입: 유료 tier 검토 필요
- 동시 사용자 50명+: 인프라 최적화 필요
- 2년 이상 운영: 데이터 아카이빙 전략 수립

---

## 10. 체크리스트

- [x]  React 웹 애플리케이션 기본 구조
- [x]  Face-api.js 얼굴 인식 기능
- [x]  Firebase Firestore 데이터 저장
- [x]  Google Apps Script 자동화 스크립트
- [x]  Google Sheets 백업 기능
- [x]  오류 처리 및 타임아웃 알림
- [x]  Vercel 배포 설정 및 자동화

---
