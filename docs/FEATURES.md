...
**문서명**: 방문자 체크인 시스템 기능 정리

**작성일**: 2026-01-28

**수정일**: 2026-02-11

> 공개 저장소용으로 정리한 기능 문서입니다. 실제 현장 운영 절차를 그대로 설명하기보다는, 개인 프로젝트에서 구현한 기능과 예시 시나리오를 중심으로 작성했습니다.

---

## 2. 기능 개요

본 시스템은 5개의 주요 모듈로 구성되며, 각 모듈은 독립적으로 동작하면서 방문자 입력과 집계를 돕는 데모 흐름을 제공합니다.

> 전체 시스템 구조는 [ARCHITECTURE.md](./ARCHITECTURE.md)를 참조하세요.
...
- 성별 토글 버튼 (남성/여성)
- 연령대 그리드 버튼 (유아/어린이/청소년/청년/중년/노년) 클릭으로 방문자 리스트에 추가
- "모드 전환" 버튼으로 AI 모드 진입
...
- 공간 선택 (Firebase 'locations' 컬렉션에서 로드, localStorage 캐시)
- PIN 비밀번호 인증
- localStorage에 설정 저장
- HTTPS 전송, 클라이언트 사이드 검증
- 공간 목록 로드: 캐시 우선, 없으면 Firebase에서 실시간 로드

### 1.5 방문자 리스트

**기능**:

- 동일 성별/연령대 자동 그룹화 (예: 청년 남성 x3)
- +/- 버튼으로 수량 조절, X 버튼으로 그룹 삭제
- "초기화" 버튼으로 전체 목록 비우기
- 총 인원수 배지 표시
- "완료" 버튼으로 최종 확인 모달 열기 (Firestore 전송 전 최종 체크)
...
**목적**: 사용자가 일일 통계와 설정을 확인

**접근 방법**: 로고를 3회 연속 터치 (3초 내)

**기능**:

- **일일 통계 표시** (localStorage 기반, 공간별 필터링)
    - 오늘의 누적 입장객 수 (현재 공간)
    - 연령대별 분포 (막대 차트)
    - 성별 분포 (도넛 차트 + 비율 표시)
    - 앱 실행 시 이전 날짜 데이터 자동 정리
- **설정 관리**
    - 공간 선택 (드롭다운)
        - Firebase 'locations' 컬렉션 기반 동적 목록
        - 로드: 캐시 우선 조회, 없으면 Firebase에서 실시간 로드
    - 공간 관리 버튼 → RoomManagementModal 열림
        - 새 공간 추가: Firebase에 저장, 선택지 즉시 반영
        - 공간 삭제: 현재 선택 중인 항목 제외 가능, 삭제 확인 모달
...
- 최종 확인 모달 (FinalConfirmModal)
    - **표시 조건**: AI 스캔 후 또는 수동 입력 완료 후 최종 확인 필요
    - **표시 내용**: 그룹화된 방문객 목록 (같은 성별/연령대별 카운팅)
    - **버튼**:
        - "취소": 입력 결과 취소 (모달 닫기)
        - "확인": 입력 결과를 즉시 Firestore에 전송
    - **안내문**: "정보가 정확한지 확인해주세요"
- 성공 모달 (SuccessModal)
    - **표시 조건**: 데이터 전송 성공 시
    - **기능**:
...
├── source: "AI" | "수동"
├── location: "space-a" | "space-b" | ...

locations/ (공간 관리)
├── document ID: 자동 생성
├── name: "space-a" | "space-b" | ... (동적 관리)
...
**목적**: Firestore 데이터를 Google Sheets에 정기적으로 백업하는 예시 자동화 흐름 제공
...
**파일 위치**: `./docs/script.txt`

- 저장소에 포함된 파일은 예시 템플릿이며 실제 운영 계정 정보는 포함하지 않음
...
```
2026-01-28 14:30:45 | 남성 | 청년(20~39세) | AI | 다목적실-1
2026-01-28 14:35:12 | 여성 | 중년(40~64세) | 수동 | 상설전시
